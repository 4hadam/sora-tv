...
function buildCountryMesh(feature: any, selected: boolean): THREE.Object3D {
  ...
  for (const poly of polys) {
    const ring = poly[0]
    const pts3d = ring.map(([lon, lat]: number[]) => latLonToVec3(lat, lon, GLOBE_R * ALT))

    const verts: number[] = []
    const cx = pts3d.reduce((s: number, v: THREE.Vector3) => s+v.x, 0) / pts3d.length
    const cy = pts3d.reduce((s: number, v: THREE.Vector3) => s+v.y, 0) / pts3d.length
    const cz = pts3d.reduce((s: number, v: THREE.Vector3) => s+v.z, 0) / pts3d.length
    const cen = new THREE.Vector3(cx,cy,cz).normalize().multiplyScalar(GLOBE_R * ALT)

    for (let i = 0; i < pts3d.length - 1; i++) {
      verts.push(cen.x, cen.y, cen.z)
    ...
    const lineGeo = new THREE.BufferGeometry().setFromPoints(
      pts3d.map((v: THREE.Vector3) => new THREE.Vector3(v.x, v.y, v.z))
    )
